...
function RequestDetails() {
  ...
  const requestData = allRequestData.find(
  ...
  function StatusButton(status) {
    if (status === "đang chờ chấp nhận") {
      return <button onClick={handleCancelOrder}>Hủy đơn hàng</button>;
  ...
  return (
    <div className="informationForm-container">
      <h2 className="informationForm-name">
        Phiếu yêu cầu sửa chữa số {parseInt(requestId)}
      </h2>
      <form aria-readonly>
